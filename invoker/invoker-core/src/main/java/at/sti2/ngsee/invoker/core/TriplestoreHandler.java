...
public class TriplestoreHandler {
	...
	/**
	 * TODO: Only IN_OUT is supported. Make inputMessage and outputMessage
	 * optional to support also IN_ONLY and OUT_ONLY.
	 * 
	 * @param _serviceID
	...
	public static InvokerMSM getInvokerMSM(String _serviceID,
		...
		if (result.hasNext()) {
			...
			String namespace = entry.getBinding("namespace").getValue()
					.stringValue();

			msmInstance.setLiftingSchema(new URL(entry.getBinding("lifting")
					.getValue().stringValue()));
			msmInstance.setLoweringSchema(new URL(entry.getBinding("lowering")
					.getValue().stringValue()));
			msmInstance.setWSDL(new URL(entry.getBinding("wsdl").getValue()
					.stringValue()));
			msmInstance.setOperationQName(new QName(namespace, _operationName));
			msmInstance.setServiceQName(new QName(namespace, entry
					.getBinding("servicename").getValue().stringValue()));
			msmInstance.setPortQName(new QName(namespace, entry
					.getBinding("portname").getValue().stringValue()));
			Binding soapAction = entry.getBinding("soapaction");
			if (null != soapAction)
				msmInstance.setSOAPAction(soapAction.getValue().stringValue());
			msmInstance.setEndpointURL(new URL(entry.getBinding("endpoint")
					.getValue().stringValue()));
